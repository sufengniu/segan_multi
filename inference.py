...
flags.DEFINE_string("save_path", "/zfs/dodcard/models/segan_v1.1/", "Path to save out model "
...
flags.DEFINE_string("synthesis_path", "dwavegan_samples", "Path to save output"
                                                          ...
                                                          " (Def: dwavegan_sam"
                                                          "ples).")
flags.DEFINE_string("e2e_dataset", "/scratch2/sniu/deepspeech/data/segan.tfrecords", "TFRecords"
                                                          " (Def: data/"
                                                          "segan.tfrecords.")
flags.DEFINE_string("save_clean_path", "/zfs/dodcard/dataset/noise_data/", "Path to save clean utts")
flags.DEFINE_string("test_wav", "/zfs/dodcard/dataset/noise_data/Switchboard_with_noise/sw02001-1.wav", "name of test wav (it won't train)")
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        if FLAGS.model == 'gan':
            ...
            se_model = SEGAN(sess, FLAGS, udevices, infer=True)
        elif FLAGS.model == 'ae':
            print('Creating AE model')
            se_model = SEAE(sess, FLAGS, udevices, infer=True)
        else:
