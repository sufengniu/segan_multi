...
def read_and_decode(filename_queue, canvas_size, preemph=0.):
    ...
    features = tf.parse_single_example(
            ...
            features={
                ...
                'noisy_only_raw': tf.FixedLenFeature([], tf.string),
            })
    ...
    noisy_only = tf.decode_raw(features['noisy_only_raw'], tf.int32)
    noisy_only.set_shape(canvas_size)
    noisy_only = (2./65535.) * tf.cast((noisy_only - 32767), tf.float32) + 1.
    if preemph > 0:
        wave = tf.cast(pre_emph(wave, preemph), tf.float32)
        noisy = tf.cast(pre_emph(noisy, preemph), tf.float32)
        noisy_only = tf.cast(pre_emph(noisy_only, preemph), tf.float32)

    return wave, noisy, noisy_only
