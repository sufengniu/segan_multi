...
#             if self.decoder_type != 'III':
#                 self.reference_G1 = ref_Gs[0]
#             elif self.decoder_type != 'I' and self.decoder_type != 'II':
...
#         if self.decoder_type != 'I' and self.decoder_type != 'II':
#             self.Gs2.append(G2)
#             self.zs2.append(z2)
...
#         if self.decoder_type == 'I' or self.decoder_type == 'II':
#             d_loss = d_rl_loss + d_fk_loss
#         elif self.decoder_type == 'III':
...
#         else:
#             d = tf.sqrt(tf.reduce_sum(tf.pow(H1-H2, 2), 1, keep_dims=True))
#             margin = 10.0
...
#         if config.decoder_type == 'IV' or config.decoder_type == 'V':
#             g_summs += [self.hinge_loss_sum]

#         if config.decoder_type != 'III':
...
#                        self.gen_summ,
#                        self.gen_audio_summ]
#             d_sums += [self.d_rl_sum,
#                        self.d_rl_loss_sum]
#         if config.decoder_type != 'I' and config.decoder_type != 'II':
#             g_summs += [self.d_fk_noise_sum,
...
#         # pick a single batch
#         sample_z1, sample_z2 = None, None
#         if self.decoder_type == 'I' or self.decoder_type == 'II':
#             sample_noisy, sample_wav, sample_noise_only, \
#             sample_z1 = self.sess.run([self.gtruth_noisy[0],
#                                       self.gtruth_wavs[0],
...
#                     _, g_adv_only_loss, g_l1_only_loss, g_adv_loss, g_l1_loss = g_out
#                     d_fk_losses.append(d_fk_loss)
#                     d_fk_only_losses.append(d_fk_only_loss)
...
#                       'd_rl_only_loss = {:.5f}, d_fk_loss = {:.5f},'
...
#                     if sample_z1 != None:
#                         fdict[self.zs1[0]] = sample_z1
#                     if sample_z2 != None:
#                         fdict[self.zs2[0]] = sample_z2
                ...
                if (counter / num_devices) % config.save_freq == 0:
                    ...
                    for m in range(min(20, canvas_w.shape[0])):
                        ...
                        if not os.path.exists(os.path.join(save_path, 'gtruth_{}.wav'.format(m))):
                            ...
                            wavfile.write(os.path.join(save_path, 'noisy_{}.wav'.format(m)), int(16e3), sample_noisy[m])
                            wavfile.write(os.path.join(save_path, 'dif_{}.wav'.format(m)), int(16e3), sample_dif[m])
